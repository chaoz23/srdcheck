...
- **Scope travels with the verdict.** Verdict schema 2.0 requires a named
  `coverage_level`, `checked_scope`, `unchecked_scope`, and `assumptions` on
  every result. Every legal explanation explicitly says it is legal only within
  that checked scope.
...
|---|---|---|---|
| T1 | wrong verdicts | enforced in CI | 654 tests including gold suites ported from the Phase 0 eval; any wrong verdict fails the build |
...
| T10 | stranger-agent bootstrap | enforced in CI | cold-start conformance test reaches a first verdict from tool.json/--schema/MCP alone (23 tools); live probe: a frontier model given only tool.json produced a correct first verdict in 1 attempt(s), 3.9s (2026-07-16) |
