...
Adapter identifiers carry their version, so `load_adapter("srd-5.2.1")` and a future `load_adapter("srd-5.1")` coexist without a breaking change. The handle is content-neutral: it knows about categories and records, never about a specific ruleset's vocabulary.
...
|---|---|---|---|
| T1 | wrong verdicts | enforced in CI | 101 tests including gold suites ported from the Phase 0 eval; any wrong verdict fails the build |
