...
|---|---|---|---|
| T1 | wrong verdicts | enforced in CI | 84 tests including gold suites ported from the Phase 0 eval; any wrong verdict fails the build |
...
| T10 | stranger-agent bootstrap | enforced in CI | cold-start conformance test reaches a first verdict from tool.json/--schema/MCP alone (12 tools); live probe: a frontier model given only tool.json produced a correct first verdict in 1 attempt(s), 3.9s (2026-07-16) |
