...
|---|---|---|---|
| T1 | wrong verdicts | enforced in CI | 214 tests including gold suites ported from the Phase 0 eval; any wrong verdict fails the build |
| T2 | no citation, no rule | enforced in CI | 84/84 rule atoms carry verbatim source quotes; every-verdict-cites tests on all adjudicated paths |
...
| T10 | stranger-agent bootstrap | enforced in CI | cold-start conformance test reaches a first verdict from tool.json/--schema/MCP alone (18 tools); live probe: a frontier model given only tool.json produced a correct first verdict in 1 attempt(s), 3.9s (2026-07-16) |
