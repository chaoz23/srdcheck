...
|---|---|---|---|
| T1 | wrong verdicts | enforced in CI | 57 tests including gold suites ported from the Phase 0 eval; any wrong verdict fails the build |
...
| T6 | judge, never simulate | enforced in CI | determinism test (identical verdicts across repeated queries); kernel verdict path holds no RNG, network, or owned state |
...
| T10 | stranger-agent bootstrap | partial | CLI, --pipe, --schema, tool.json, pip install, and an MCP server exposing 9 tools are shipped; the standing cold-start agent eval is not built yet |
