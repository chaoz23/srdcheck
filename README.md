...
|---|---|---|---|
| T1 | wrong verdicts | enforced in CI | 178 tests including gold suites ported from the Phase 0 eval; any wrong verdict fails the build |
| T2 | no citation, no rule | enforced in CI | 68/68 rule atoms carry verbatim source quotes; every-verdict-cites tests on all adjudicated paths |
