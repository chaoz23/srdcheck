...
- **Conditions beyond** Grappled, Prone, Incapacitated, Invisible, Blinded, Restrained, Stunned, Paralyzed. Any other condition is refused (exit 2), never guessed — including Unconscious, which the reducer *sets* when a creature drops to 0 HP but whose attack/turn effects the adjudication queries don't yet model.
...
|---|---|---|---|
| T1 | wrong verdicts | enforced in CI | 157 tests including gold suites ported from the Phase 0 eval; any wrong verdict fails the build |
| T2 | no citation, no rule | enforced in CI | 49/49 rule atoms carry verbatim source quotes; every-verdict-cites tests on all adjudicated paths |
...
| T13 | the benchmark is a product | shipped | bench/ publishes 4 sets across 5 subjects with cited gold verdicts, incl. a per-horizon drift lane whose golds are engine-derived; a generated LEADERBOARD ranks any subject; cmd: driver + validate command let a third party submit a tamper-checked result |
