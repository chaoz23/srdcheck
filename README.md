...
non-clean. Rule and citation evidence stays attached to findings. Every result
is `self_attested`: this adapter cannot grant table, encounter-state, action,
or protected-host authority.
...
ruling, or stop—without parsing `why`. When DM authority is required, an
...
|---|---|---|---|
| T1 | wrong verdicts | enforced in CI | 644 tests including gold suites ported from the Phase 0 eval; any wrong verdict fails the build |
