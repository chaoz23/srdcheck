...
- **Content outside the SRD 5.2.1** — anything not in the SRD returns exit 2 from `jurisdiction`; in adapter 0.2.1 the name registries cover spells, creatures, conditions, **classes, subclasses, species, and feats** (census-anchored against the source text in CI).
...
|---|---|---|---|
| T1 | wrong verdicts | enforced in CI | 372 tests including gold suites ported from the Phase 0 eval; any wrong verdict fails the build |
