...
1. **The exit code IS the verdict, and it is scoped:** 0 passes the named
   checked scope · 1 conflicts with it · 2 cannot-adjudicate. "Legal" means
   legal *within the verdict's `checked_scope`* — never global legality;
   `unchecked_scope` lists what was not examined.
...
3. **Cite what it cites.** Applied rules carry verbatim SRD quotes — relay
   the citation, don't paraphrase from your own rules memory. Boundary
   refusals carry no citations, on purpose.
...
srdcheck query ... --trace [--request-id X] # + observability events (request/verdict ids, versions, timing)
srdcheck policy validate <manifest>         # portable DM table-policy manifests
...
```json
{
  "verdict": "legal",
  "exit_code": 0,
  "why": "Grapple is possible: the target makes a Strength or Dexterity save (its choice) vs DC 13; on a failure, the Grappled condition. Legal only within this checked scope.",
  "citations": [{
  ...
  "checked_scope": ["size legality", "free hand", "save DC formula"],
  "unchecked_scope": ["action economy this turn", "rolling dice"],
  "data": {"kind": "grapple", "dc": 13, "save_ability": "str-or-dex (target's choice)", "on_fail": "the Grappled condition"}
}
```

(Verdict schema 4.0 also reports `facts` with per-field source/confidence,
an advisory `rule_result`, any scoped DM `table_decision`, and explicit
`state_mutation` — elided above.)
...
- MUST NOT adjudicate homebrew/third-party content by analogy; that's the
  DM's call — record it as a table policy (`policy validate`), which applies
  with explicit table-authority lineage, distinct from the SRD result.
- MUST NOT relay "legal" as blanket permission: state it as passing the
  checked scope, and surface `unchecked_scope` when the table might assume
  more was verified.
...
- **0.9.x:** verdict schema 4.0 — every result carries `coverage_level`,
  `checked_scope`, `unchecked_scope`, `assumptions`, and per-fact
  provenance; "legal" is always scoped. `--trace` emits metadata-only
  observability events (request/verdict sha256 ids, versions, timing —
  never prompts, state, or rulings). Portable DM **table policies**
  (`policy validate|export`) persist rulings with authority lineage.
  `event.apply` → `transition.commit` state changes fail closed. If you
  hold 0.5.x-era knowledge of flat verdicts, that's stale.
- **0.5.x:** default adapter became **srd-5.2.1 (2024 rules)**; srd-5.1
  (2014) is non-default — verify 2014-era knowledge with
  `edition-check "<name>"`. Adapter kit: `conformance`, `new-adapter`.
